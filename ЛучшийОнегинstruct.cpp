...
int main()
    {
    FILE* reading = fopen("C:\\Users\\pomah_000\\Desktop\\Onegin\\reading.txt" , "r");
    assert(reading);
   ...
   // char* reversesort[string_counter-1];
    ...
    struct pointer pointers_reverse_sort_struct[string_counter-1];
    for (int i=0; i<string_counter; i++)
        {
        pointers_reverse_sort[i] = &pointers_reverse_sort_struct[i] ;
        }
    ...
    qsort (pointers_reverse_sort, string_counter, sizeof(pointers_reverse_sort[0]), my_reverse_strcmp);
...
int my_reverse_strcmp(const void *string1, const void *string2)
    ...
    char* string_a =  (*(struct pointer **)string1) ->ending;
    char* string_b =  (*(struct pointer **)string2) ->ending;
...
int is_rus_letter(char letter)
    {
    if ((letter >= '¿' && letter <= 'ﬂ') || (letter >= '‡' && letter <= 'ˇ') )
                {
                return 1;
                }
    else
